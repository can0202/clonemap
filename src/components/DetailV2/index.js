...
const DetailV2Component = ({
  ...
  // Xử lý reload lại trang để lấy token từ server
  const authCodeParam = router.query.authCode;
  useEffect(() => {
    if (isWaitingToken && !authCodeParam) {
  ...
  const typeLink = typesReplaceMapping[postsData?.postType?.code];
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: 'Trang chủ',
        item: 'https://varsland.vn/',
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: postsData?.postType?.name,
        item: `https://varsland.vn/${typeLink}`,
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: postsData?.typeRealEstate?.name,
        item: `https://varsland.vn/${postsData?.typeRealEstate?.fields?.slug}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: postsData?.province?.name,
        item: `https://varsland.vn/${postsData?.typeRealEstate?.fields?.slug}-tai-${postsData?.province?.slug}-t${postsData?.province?.code}`,
      },
      {
        '@type': 'ListItem',
        position: 5,
        name: postsData?.district?.name,
        item: `https://varsland.vn/${postsData?.typeRealEstate?.fields?.slug}-tai-${postsData?.district?.slug}-t${postsData?.province?.code}`,
      },
      {
        '@type': 'ListItem',
        position: 6,
        name: postsData?.wards?.name,
        item: `https://varsland.vn/${postsData?.typeRealEstate?.fields?.slug}-tai-${postsData?.wards?.slug}-t${postsData?.province?.code}`,
      },
    ],
  };
...
export default DetailV2Component;
DetailV2Component.propTypes = {
