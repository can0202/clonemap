...
const SaleBDS = asyncComponent(() => import('../../modules/bds-ban/index'), {
  ssr: true,
});
export default AppPage(({postsCate, url}) => (
  <SaleBDS postsCate={postsCate} url={url} />
));
...
export const getServerSideProps = async ({req, resolvedUrl}) => {
  function getURLParameters(routeUrl) {
    const queryString = routeUrl?.split('?')[1];
    if (!queryString) {
      return {};
    }
